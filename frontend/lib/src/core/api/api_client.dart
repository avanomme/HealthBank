...
import 'mobile_auth_interceptor.dart';

/// API configuration constants
class ApiConfig {
  // Local dev override — only active when --dart-define=API_BASE_URL is passed.
  // Production builds use the default relative path (no override needed).
...
class ApiClient {
    ...
    final dio = Dio(
      BaseOptions(
        ...
        headers: {
          ...
          // Remove on Prod(HTTPS): Remove once HTTPS + SameSite=None;Secure cookies are
          // in place. This header tells the backend to include the session token
          // in the response body so the MobileAuthInterceptor can use Bearer
          // auth for cross-origin dev builds and native mobile. Same-origin
          // production builds never set API_BASE_URL so never send this header.
          if (ApiConfig.baseUrl.isNotEmpty) 'X-Client-Type': 'native',
        },
        // Same-origin web builds use cookies (withCredentials). Cross-origin
        // dev builds (API_BASE_URL set) use Bearer via MobileAuthInterceptor.
        extra: (kIsWeb && ApiConfig.baseUrl.isEmpty) ? {'withCredentials': true} : {},
      ),
    );
  ...
  /// Install Bearer token interceptor. Call once after ProviderScope is
  /// available.
  ///
  /// Active on:
  ///   - All mobile platforms (Android / iOS / desktop)
  ///   - Web when API_BASE_URL is explicitly set (cross-origin dev mode)
  ///
  /// No-op when running as same-origin web (deployed build served through
  /// nginx) — in that case the browser sends HttpOnly cookies automatically.
  static void installMobileAuth(WidgetRef ref) {
    final crossOriginWeb = kIsWeb && ApiConfig.baseUrl.isNotEmpty;
    if (!kIsWeb || crossOriginWeb) {
      final container = ProviderScope.containerOf(ref.context);
      final instance = ApiClient();
      instance._dio.interceptors.insert(
        0,
        MobileAuthInterceptor(container),
      );
    }
  }
